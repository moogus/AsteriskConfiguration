...
CREATE ALGORITHM=UNDEFINED DEFINER=`asterisk`@`%` SQL SECURITY DEFINER VIEW `ast_queue_members` AS 
		
		select `com_queueMembers`.`id` AS `uniqueid`,
				(case `com_queueMembers`.`Type` 
				when 1 then concat('SIP/',`extension`.`Number`) 
				when 2 then concat('local/',`queue`.`Number`,'@LocalSets') else '' end) AS `membername`,
				`parent`.`Number` AS `queue_name`,
				(case `com_queueMembers`.`Type` 
				when 1 then concat('SIP/',`extension`.`Number`) 
				when 2 then concat('local/',`queue`.`Number`,'@LocalSets') else '' end) AS `interface`,
				`com_queueMembers`.`Penalty` AS `penalty`,`com_queueMembers`.`Paused` AS `paused` 
		from (((`com_queueMembers` 
				left join `com_extensions` `extension` 
				on((`com_queueMembers`.`ExtensionId` = `extension`.`id`))) 
				left join `com_queues` `parent` on((`com_queueMembers`.`ParentQueueId` = `parent`.`id`))) 	
				left join `com_queues` `queue` 
				on((`com_queueMembers`.`QueueId` = `queue`.`id`)));



CREATE ALGORITHM=UNDEFINED DEFINER=`asterisk`@`%` SQL SECURITY DEFINER VIEW `ast_queues` AS 

		select 	`com_queues`.`id` AS `id`,
				`com_queues`.`Number` AS `name`,
				`com_musiconhold`.`name` AS `musiconhold`,
				'' AS `announce`,
				'' AS `context`,
				'' AS `timeout`,
				'' AS `monitor_join`,	
				...
				'' AS `queue_thankyou`,
				'' AS `queue_reporthold`,
				'' AS `announce_frequency`,
				'' AS `announce_round_seconds`,
				'' AS `announce_holdtime`,
				'' AS `retry`,
				'' AS `wrapuptime`,
				'' AS `maxlen`,
				'' AS `servicelevel`,
				lcase(`com_queues`.`Strategy`) AS `strategy`,
			(case `com_queues`.`RingOnBusy` 
			when 1 then 'paused,inuse' 
			else 'paused' end) AS `joinempty`,
				'' AS `leavewhenempty`,
				'' AS `eventmemberstatus`,
				'' AS `eventwhencalled`,
				'' AS `reportholdtime`,
				'' AS `memberdelay`,
				'' AS `weight`,
				'' AS `timeoutrestart`,
				'' AS `periodic_announce`,
				'' AS `periodic_announce_frequency`,
				'' AS `ringinuse`,
				'' AS `setinterfacevar` 
			from
				(`com_queues` 
			join `com_musiconhold` on((`com_queues`.`com_musiconholdId` = `com_musiconhold`.`id`)));



CREATE ALGORITHM=UNDEFINED DEFINER=`asterisk`@`%` SQL SECURITY DEFINER VIEW `ast_sipfriends` AS 
			select 
				`com_extensions`.`id` AS `id`,
				'friend' AS `type`,
				`com_extensions`.`Number` AS `name`,
				`com_extensions`.`Password` AS
				`secret`,'LocalSets' AS `context`,
				'Dynamic' AS `host`,
				`com_extensions`.`Number` AS `defaultuser`,
				'' AS `insecure`,
				'BLF_Group_1' AS `subscribecontext`,
				'1' AS `callgroup`,
				'1' AS `pickupgroup`,
				'yes' AS `allowsubscribe`,
				'yes' AS `notifyringing`,
				'yes' AS `notifyhold`,'yes' AS `notifycid`,
				'' AS `port`,
				`ast_voicemail`.`mailbox` AS `mailbox`,
			(select `com_server`.`voicemailNumber` from `com_server` limit 0,1) AS `vmexten`
			from
				(`com_extensions` 
			left join `ast_voicemail` 
			on((`com_extensions`.`VoiceMailId` = `ast_voicemail`.`uniqueid`))) 
			where (`com_extensions`.`Number` <> 'balls')
			union 
			select 
				`com_extensions`.`id` AS `id`,
				'friend' AS `type`,
				`com_extensions`.`Number` AS `name`,
				`com_extensions`.`Password` AS `secret`,
				'Trunks' AS `context`,
				'Dynamic' AS `host`,
				`com_extensions`.`Number` AS `defaultuser`,
				'' AS `insecure`,
			...
			from
				`com_extensions` 
			where 
				(`com_extensions`.`Number` = 'balls') 
			union 
			select 
				`com_trunk`.`id` AS `id`,
				'peer' AS `peer`,
				`com_trunk`.`TrunkName` AS `name`,
				`com_sipTrunkCredentials`.`Password` AS `secret`,
				'Trunks' AS `context`,
				`com_sipTrunkCredentials`.`Host` AS `host`,
				`com_sipTrunkCredentials`.`UserName` AS `defaultuser`,
				'invite' AS `insecure`,
				'' AS `subscribecontext`,
				'' AS `callgroup`,
				'' AS `pickupgroup`,
				...
				'' AS `vmexten` 
			from 
				(`com_sipTrunkCredentials` 
			left join `com_trunk` 
			on((`com_sipTrunkCredentials`.`trunkId` = `com_trunk`.`id`)));



CREATE ALGORITHM=UNDEFINED DEFINER=`asterisk`@`%` SQL SECURITY DEFINER VIEW `com_iaxFriends` AS
		
			select 
				`fu_IaxCredentials`.`name` AS `name`,
				`fu_IaxCredentials`.`host` AS `host`,
				'' AS `port`,
				'' AS `ipaddr`,
				'' AS `secret`,
				`fu_IaxCredentials`.`name` AS  `peername`,
				`fu_IaxCredentials`.`name` AS `username`,
				`fu_IaxCredentials`.`com_trunkId` AS `com_trunkId`,
				'friend' AS `type`,
				'yes' AS `trunk`,
				'incoming' AS  `context`,
				'' AS `regseconds` 
			from `fu_IaxCredentials`;
